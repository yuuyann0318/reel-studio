...
def _track_contract_section():
    ...
    return (
        ...
        "| V2 | テロップ（各ショット区間に直接配置） | Iris | shot 数と同じだけの generatoritem |\n"
        "| V3 | 演出（赤丸オーバーレイ等） | Rose | `RC01 red_circle` |\n"
        "| A1 | ナレーション | Forest | `NAR narration` |\n"
        "| A2 | BGM（キーフレーム音量カーブ付き） | Lavender | `BGM bgm` |\n"
        ...
        "### V2 テロップと SRT の使い分け\n\n"
        "reel.xml の V2 トラックには、各ショット表示区間の見た目デフォルト（Noto Sans JP Black・"
        "白文字＋黒縁取り・下部セーフエリア）で `generatoritem` が並んでいます。ここは Premiere で"
        "そのまま編集できます。同梱の `captions.srt` はキャプション形式で編集したい場合の代替経路"
        "（ドラッグ&ドロップでキャプショントラック化）です。V2 と SRT を両方使うと二重表示に"
        "なるため、どちらか一方だけ有効にしてください。\n\n"
        "### BGM のキーフレーム音量カーブ\n\n"
